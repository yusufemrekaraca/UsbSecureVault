...
public static class CryptoService
{
    ...
    public static void EncryptFileToPath(byte[] key, string sourcePath, string destinationPath, out string nonceBase64, out string tagBase64, IProgress<long>? progress = null)
    {
        var tempPath = Path.Combine(
            Path.GetDirectoryName(destinationPath)!,
            $".{Path.GetFileName(destinationPath)}.{Guid.NewGuid():N}.usvtmp");

        try
        {
            EncryptFileFromPathToPath(key, sourcePath, tempPath, out nonceBase64, out tagBase64, progress);
    ...
    public static void EncryptFileFromPathToPath(byte[] key, string sourcePath, string destinationPath, out string nonceBase64, out string tagBase64, IProgress<long>? progress = null)
    {
        EncryptFileChunked(key, sourcePath, destinationPath, progress);
        nonceBase64 = "";
        tagBase64 = "";
    }
    ...
    private static void EncryptFileChunked(byte[] key, string sourcePath, string destinationPath, IProgress<long>? progress)
    {
